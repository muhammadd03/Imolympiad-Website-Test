...
output_file = "RegisterAsParticipant.txt"
...
with open(output_file, "w") as f:
    ...
    #For register as participant
    try:
        participant = WebDriverWait(driver, 30).until(EC.element_to_be_clickable((By.CSS_SELECTOR, 'input.form-check-input.RegisterAs[type=radio][value=participant]#RegisterAsParticipant')))
        ActionChains(driver).move_to_element(participant).perform()
        participant.click()
        print('The Participant radio button is clickable')
    except:
        print('The Participant radio button is not clickable')
